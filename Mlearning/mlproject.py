...
importance = pd.DataFrame(
    {"특성": X.columns, "중요도": best_model.feature_importances_}
...
study_hour = st.number_input(
    "하루 공부시간", min_value=0.00, max_value=10.00, help="최대 10시간까지 입력하세요"
)
social_media = st.number_input(
    "하루 SNS 시간", min_value=0.00, max_value=10.00, help="최대 10시간까지 입력하세요"
)
sleep_hour = st.number_input(
    "하루 수면 시간", min_value=0.00, max_value=10.00, help="최대 10시간까지 입력하세요"
)
exercise_frequency = st.number_input(
    "일주일 간 운동횟수", min_value=0.00, max_value=7.00, help="최대 7번까지 입력하세요"
)
